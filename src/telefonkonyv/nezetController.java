...
public class nezetController implements Initializable {
    ...
    private final ObservableList<szemely> data = FXCollections.observableArrayList();

    @FXML
    private void addContact(ActionEvent event) {
        String email = inputEmail.getText();
        if (email.length() > 3 && email.contains("@") && email.contains(".")) {
            szemely newSzemely = new szemely(inputLastname.getText(), inputFirstName.getText(), email);
            data.add(newSzemely);
            adatbazis.addContact(newSzemely);
            inputLastname.clear();
    ...
    private void exportList(ActionEvent event) {
        ...
        if (fileName != null && !fileName.equals("")) {
            generator pdfCreator = new generator();
            pdfCreator.generator(fileName, data);
        }else{
    ...
    public void setTableData() {
        ...
        lastNameCol.setCellValueFactory(new PropertyValueFactory<szemely, String>("lastName"));

        lastNameCol.setOnEditCommit(
                new EventHandler<TableColumn.CellEditEvent<szemely, String>>() {
            @Override
            public void handle(TableColumn.CellEditEvent<szemely, String> t) {
                szemely actualPerson = (szemely) t.getTableView().getItems().get(t.getTablePosition().getRow());
                actualPerson.setLastName(t.getNewValue());
                adatbazis.updateContact(actualPerson);
            }
        }
        );
        ...
        firstNameCol.setCellValueFactory(new PropertyValueFactory<szemely, String>("firstName"));
        ...
        emailCol.setCellValueFactory(new PropertyValueFactory<szemely, String>("email"));
                            ...
                            public void updateItem( String item, boolean empty )
                            {
                                ...
                                else
                                {
                                    btn.setOnAction( ( ActionEvent event ) ->
                                            {
                                                szemely person = getTableView().getItems().get( getIndex() );
                                                data.remove(person);
                                                adatbazis.removeContact(person);
                                       } );
                                    setGraphic( btn );
                                    setText( null );
                                }
        ...
        data.addAll(adatbazis.getAllContacts());
